...
def get_drone_feature(modelo):
    try:
        ...
        raise ValueError(f"Arquivo de dados de drone não encontrado: {DRONE_DATA_PATH}")
    except json.JSONDecodeError:
        raise ValueError(f"Erro ao decodificar JSON em {DRONE_DATA_PATH}. Verifique a formatação.")

    dados = drone_data.get(modelo)
    if not dados:
        raise ValueError(f"Modelo de drone '{modelo}' não encontrado nos dados.")
...
class GlobalSettings:
    def __init__(self):
        ...
        # self.drone_data is no longer needed here as get_drone_feature loads it

    def _validate_project(self):
        return self.project.customVariables().get("own") == "GeoOne"

    # _load_drone_data and _get_sensor_layer are no longer needed

def create_add_sensor_section(dock):
...
def calculate_and_add_drone(dock):
    try:
        ...
        with open(DRONE_DATA_PATH, 'w', encoding='utf-8') as f:
